    ...
    >
      ...
      >
        ...
        <div className="px-5 py-4 border-b border-cream-border shrink-0">
          <h3 className="text-base md:text-sm font-semibold text-foreground">Add Bulk Dispatch</h3>
          <p className="text-xs text-gray-500 mt-0.5">
            Select an event, add items + quantities, then submit to bulk-mark units dispatched in chronological order.
          </p>
        </div>
          ...
          {result?.type === "success" && (
            <div className="mb-4 flex items-start gap-2 px-3 py-2.5 rounded-lg bg-green-50 border border-green-200">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium text-green-800">Dispatch processed</div>
                <div className="text-xs text-green-700 mt-0.5">
                  {(() => {
                    ...
                    return `${result.results.length} item${result.results.length === 1 ? "" : "s"} · ${totalOrders} order${totalOrders === 1 ? "" : "s"} updated${totalExcess > 0 ? ` · ${totalExcess} unfilled` : ""}`
                  })()}
                </div>
              </div>
            </div>
          )}
          <form onSubmit={handleSubmit} className="space-y-5" id="dispatch-form">
            {/* Event */}
            <div>
            ...
            <div>
              <label className={LABEL}>
                Dispatch tracking <span className="text-gray-400 font-normal text-xs">(optional)</span>
              </label>
              <input
                type="text"
                value={tracking}
                onChange={(e) => { setTracking(e.target.value); setResult(null) }}
                  ...
                  {itemResult.excess > 0 && (
                    <div className="px-4 py-2 border-b border-cream-border bg-yellow-50 flex items-center gap-2">
                      ...
                      <span className="text-xs text-yellow-700">
                        <strong>{itemResult.excess}</strong> unit{itemResult.excess === 1 ? "" : "s"} requested beyond what&rsquo;s eligible to dispatch — left unfilled
                      </span>
                    </div>
                  )}
                  {itemResult.rows.length === 0 ? (
                    ...
                    <DispatchResultTable rows={itemResult.rows} />
                  )}
                </div>
              ))}
            </div>
        ...
        <div className="px-5 pt-3 pb-8 md:py-3 border-t border-cream-border shrink-0 flex items-center justify-end gap-2">
          ...
          >
            {submitting ? "Processing…" : result?.type === "success" ? "Process Another" : "Process Dispatch"}
          </button>
        </div>
      </div>
    </div>
...
function DispatchResultTable({ rows }: { rows: UpdatedRow[] }) {
  return (
    <table className="w-full text-sm">
      <thead>
        <tr className="border-b border-cream-border text-left">
          ...
          <th className="px-4 py-2 text-xs font-medium text-gray-500 text-right w-20">Unit Dispatch</th>
      ...
      <tbody>
        {rows.map((row, i) => (
          <tr key={row.rowNumber} className="border-b border-cream-border last:border-0">
            ...
            <td className="px-4 py-2 text-foreground text-right font-medium">{row.unitDispatch}</td>
            <td className="px-4 py-2 text-right">
              {row.oldUnitDispatch > 0 && (
                <span className="text-xs text-gray-400">(was {row.oldUnitDispatch})</span>
              )}
            </td>
          </tr>
        ))}
